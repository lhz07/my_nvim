...
-- move line
vim.keymap.set("n", "<A-,>", "<Cmd>execute 'move .+' . v:count1<CR>==", { remap = true, desc = "Toggle" })
vim.keymap.set("v", "<A-,>", ":<C-U>execute \"'<lt>,'>move '>+\" . v:count1<CR>gv=gv")
vim.keymap.set("i", "<A-,>", "<Esc><Cmd>m .+1<CR>==gi")
vim.keymap.set("n", "<A-.>", "<Cmd>execute 'move .-' . (v:count1 + 1)<CR>==")
vim.keymap.set("v", "<A-.>", ":<C-U>execute \"'<lt>,'>move '<lt>-\" . (v:count1 + 1)<CR>gv=gv")
